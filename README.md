...
`GET /signals/filtered` and `GET /regime/current` include a `parameterUncertainty` object that propagates estimation error through the full model chain. Uses Wilson score intervals on binomial hit rates (correcting small-sample bias vs naive normal approximation), t-distribution CIs on transition duration (df=1 with n=2 historical periods, t_crit=12.706), and scenario-based propagation through the EV equation. Outputs per-parameter confidence intervals, propagated 95% CI on expected return per day (aggregate + CRYPTO_LEADS), crossover day CI per type, information value ranking (which parameter improvement most narrows the crossover CI), and effective sample size diagnostic with per-module uncertainty contribution percentages. Dominant source: transition timing (56.7%, n=2). Net bias: UNDERSTATED (3 of 4 limitations push toward wider true CIs). See [`docs/PARAMETER_UNCERTAINTY.md`](docs/PARAMETER_UNCERTAINTY.md) for full derivation, field reference, and consumer code example.
...
## Prediction Calibration & Accuracy Scoring

`GET /signals/filtered` and `GET /regime/current` include a `predictionCalibration` object that converts the pipeline from a static analytics stack into a self-auditing trust instrument. Computes Brier score decomposition (reliability/resolution/uncertainty) per module, calibration slope with predicted-vs-observed binning, sharpness metric, per-module accuracy ranking with skill scores, composite pipeline calibration grade (A-F), naive baseline comparison (uniform CDF, coin flip, median step function), and leave-one-transition-out cross-validation with 95% CIs. Ground truth: 5 completed non-NEUTRAL regime transitions. See [`docs/PREDICTION_CALIBRATION.md`](docs/PREDICTION_CALIBRATION.md) for full mathematical derivation, field reference, and consumer code examples.
